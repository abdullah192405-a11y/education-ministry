...
const LessonRatingSummaryCard = ({
    ...
    const { t, dir, language } = useDashboardLocale();
    const localized = useMemo(
        () => localizeLessonRatingSummary(summary, language),
        [summary, language]
    );
    const empty = emptyMessage ?? t("dash.teacher.topics.noRatings");

    return (
        <Card
            dir={dir}
            className={className ?? "border-amber-200/60 bg-gradient-to-br from-amber-50/30 via-background to-background"}
        >
            <CardHeader className="pb-2">
                <div className="flex flex-wrap items-center justify-between gap-2">
                    <CardTitle className="text-sm">{t("dash.teacher.topics.lessonRatingTitle")}</CardTitle>
                    {localized.total > 0 && (
                        <Badge variant="secondary" className="font-bold">
                            {t("dash.teacher.topics.ratingsCountBadge", { count: localized.total })}
                        </Badge>
                    )}
                </div>
            </CardHeader>
                ...
                {localized.total === 0 ? (
                    ...
                    <>
                        <p className="mb-4 text-center text-sm text-muted-foreground">
                            {t("dash.teacher.topics.ratingsAverageLine", {
                                avg: localized.average.toFixed(1),
                            })}
                        </p>
                        <div className="grid grid-cols-5 gap-1.5 sm:gap-3">
                            {localized.distribution.map((row) => (
                                <div
                                    key={row.value}
