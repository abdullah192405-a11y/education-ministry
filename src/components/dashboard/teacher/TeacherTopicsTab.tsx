...
import type { Topic, ContentMedia } from "@/data/educationData";
...
interface TeacherTopicsTabProps {
    gradeId: number;
    subjectId: number;
    onCreateChallenge: (topicId: number) => void;
}

// Extended Topic type with status for CRUD
...
const TeacherTopicsTab = ({ gradeId, subjectId, onCreateChallenge }: TeacherTopicsTabProps) => {
    ...
    const initialTopics = (subject?.topics || []).map(topic => ({
        ...topic,
    ...
    // State
    const [topics, setTopics] = useState<ExtendedTopic[]>(initialTopics);
    ...
    const handleSaveTopic = (topicData: any) => {
        // Calculate counts
        const mediaCount = topicData.media?.length || 0;
        const quizCount = topicData.challengeItems?.length || topicData.quiz?.length || 0;
        ...
        if (editingTopic) {
            ...
            setTopics(prev => prev.map(t =>
                t.id === editingTopic.id
                    ? {
                        ...
                        mediaCount,
                        quizCount,
                        media: topicData.media || t.media,
                        quiz: topicData.quiz || t.quiz,
                        challengeItems: topicData.challengeItems || t.challengeItems
                    }
                    : t
            ));
            toast({
                title: "تم تحديث الدرس بنجاح ✓",
                description: `تم تحديث "${topicData.title}"`,
            });
        } else {
            // Create new topic
            const newTopic: ExtendedTopic = {
                id: Date.now(),
                title: topicData.title || "درس جديد",
                description: topicData.description || "",
                thumbnail: topicData.thumbnail || "https://images.unsplash.com/photo-1557804506-669a67965ba0?w=400&h=300&fit=crop",
                views: 0,
                duration: topicData.duration || "",
                createdAt: new Date().toISOString().split('T')[0],
                media: topicData.media || [],
                quiz: topicData.quiz || [],
                challengeItems: topicData.challengeItems || [],
                status: "draft",
                mediaCount,
                quizCount
            };
            ...
            toast({
                ...
                description: `تم إنشاء "${newTopic.title}"`,
            });
        }
    ...
    const handleDeleteTopic = (id: number) => {
        ...
        toast({
            title: "تم حذف الدرس",
            description: `تم حذف "${topic?.title}" بنجاح`,
            variant: "destructive",
        });
    };
                                                ...
                                                <div className="flex items-start justify-between">
                                                    <div>
                                                        <div className="flex items-center gap-2 mb-1">
                                                            ...
                                                            {topic.status === "published" && topic.id === 1 && (
