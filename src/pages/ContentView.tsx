...
import { getChannelById, getContentById, type EducationalContent, type QuizQuestion } from "@/data/channelsData";

const ContentView = () => {
    const { channelId, contentId } = useParams();
    const navigate = useNavigate();

    const channel = getChannelById(parseInt(channelId || "0"));
    const content = getContentById(parseInt(channelId || "0"), parseInt(contentId || "0"));
    ...
    useEffect(() => {
        if (content && currentMediaIndex === content.media.length - 1) {
            setHasViewedAllMedia(true);
        }
    }, [currentMediaIndex, content]);
    ...
    const totalMedia = content.media.length;
    ...
    const handleJoinChallenge = () => {
        navigate(`/channel/${channelId}/content/${contentId}/challenge`);
    };
                ...
                <div className="container mx-auto px-4">
                    ...
                    >
                        ...
                        >
                            ...
                            <span>العودة إلى {channel.name}</span>
                        </Link>
                    </motion.div>
                    ...
                    >
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className="flex items-center gap-1 text-muted-foreground text-sm">
                                ...
                                {content.views.toLocaleString("ar-SA")} مشاهدة
                            </span>
                            {content.duration && (
                                <span className="flex items-center gap-1 text-muted-foreground text-sm">
                        ...
                        <h1 className="text-3xl md:text-4xl font-black mb-3">{content.title}</h1>
