...
import {
    Users, Trophy, Gamepad2, Sparkles, ArrowLeft, ArrowRight
} from "lucide-react";
...
const JoinChallenge = () => {
    ...
    const handlePinSubmit = async () => {
        if (pin.length !== 6) {
            setError(t("joinChallenge.errPin6Digits"));
            return;
        }
        ...
        try {
            ...
            if (!directData) {
                setError(t("joinChallenge.errPinNotFound"));
                return;
            }

            if (directData.status === "FINISHED") {
                setError(t("joinChallenge.errFinished"));
                return;
            }
            ...
            if (directData.scheduled_start_time) {
                ...
                if (now < start && !isSessionHost) {
                    const formattedStart = start.toLocaleString(dateLocale, { dateStyle: "medium", timeStyle: "short" });
                    setError(
                        t("joinChallenge.errScheduledFuture", { time: formattedStart })
                    );
                    return;
                }
                ...
                if (directData.scheduled_end_time) {
                    ...
                    if (now > end) {
                        setError(t("joinChallenge.errScheduledExpired"));
                        return;
                    }
                }
    ...
    const handleJoin = async () => {
        if (!playerName.trim()) {
            setError(t("joinChallenge.errEnterName"));
            return;
        }

        if (!session || !session.id) {
            setError(t("joinChallenge.errNoSession"));
            setIsJoining(false);
            return;
        }
        ...
        try {
            ...
            if (!topicIdToUse) {
                setError(t("joinChallenge.errNoTopic"));
                return;
            }
            ...
            setError(error.message || t("joinChallenge.errPinNotFound"));
        } finally {
            setIsJoining(false);
        }
    };
                ...
                <div className="container mx-auto px-4">
                    ...
                    >
                        ...
                        <div className="text-center mb-8">
                            ...
                            <h1 className="text-3xl font-black mb-2">{t("joinChallenge.title")}</h1>
                            <p className="text-muted-foreground">
                                {step === "pin" ? t("joinChallenge.descPin") : t("joinChallenge.descName")}
                            </p>
                        </div>
                        ...
                        <Card className="p-8">
                            {step === "pin" ? (
                                ...
                                >
                                    <div className="mb-6">
                                        <label className="text-sm font-medium mb-2 block">{t("joinChallenge.pinLabel")}</label>
                                        <Input
                                            type="text"
                                            placeholder="______"
                                            value={pin}
                                ...
                                >
                                    ...
                                    <div className="mb-6 p-4 rounded-xl bg-primary/10 text-center">
                                        <div className="flex items-center justify-center gap-2 mb-2">
                                            ...
                                            <span className="font-medium">{t("joinChallenge.challengeNumber")}</span>
                                        </div>
                                        <div className="text-3xl font-black text-primary tracking-wider">{pin}</div>
                                    </div>

                                    <div className="mb-6">
                                        <label className="text-sm font-medium mb-2 block">{t("joinChallenge.nameLabel")}</label>
                                        <Input
                                            type="text"
                                            placeholder={t("joinChallenge.namePlaceholder")}
                                            value={playerName}
                                            onChange={(e) => {
                                    ...
                                    >
                                        ...
                                        {isJoining ? t("joinChallenge.joining") : t("joinChallenge.joinButton")}
                                    </Button>
                        ...
                        >
                            <div className="text-center p-4 rounded-xl bg-card border">
                                ...
                                <div className="text-xs text-muted-foreground">{t("joinChallenge.groupChallenge")}</div>
                            </div>
                            <div className="text-center p-4 rounded-xl bg-card border">
                                <Trophy className="w-6 h-6 mx-auto mb-2 text-amber-500" />
                                <div className="text-xs text-muted-foreground">{t("joinChallenge.winPrizes")}</div>
                            </div>
                            <div className="text-center p-4 rounded-xl bg-card border">
                                <Gamepad2 className="w-6 h-6 mx-auto mb-2 text-secondary" />
                                <div className="text-xs text-muted-foreground">{t("joinChallenge.funGames")}</div>
                            </div>
                        </motion.div>
                    </motion.div>
                </div>
