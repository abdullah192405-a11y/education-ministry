...
const Partners = () => {
  ...
  const partnerCards = (organizations as any[]).map((org: any) => {
    ...
    const entityType = org.entity_type || "SCHOOL";
    return {
      id: org.id,
      name: org.name,
      slug: org.slug,
      ...
      <main className="pt-32 pb-16">
        <div className="container mx-auto px-4">
          ...
          >
            <div>
              <h1 className="text-4xl md:text-5xl font-black mb-3">
                <span className="text-primary">{t("partnersPage.title")}</span>
              </h1>
              <p className="text-muted-foreground text-lg">{t("partnersPage.description")}</p>
            </div>
            <Badge variant="outline" className="text-sm">
              {partnerCards.length} {t("partnersPage.count")}
            </Badge>
          </motion.div>
          ...
          {isLoadingOrgs || isLoadingGrades ? (
            ...
            <p className="text-center text-muted-foreground py-12">{t("partnersPage.empty")}</p>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
              {partnerCards.map((org) => (
