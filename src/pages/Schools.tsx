...
const Schools = () => {
  ...
  const schoolCards = (organizations as any[])
    .filter((o) => (o.entity_type || "SCHOOL") === "SCHOOL")
    .map((org: any) => {
      ...
      <main className="pt-32 pb-16">
        <div className="container mx-auto px-4">
          ...
          >
            <div>
              <h1 className="text-4xl md:text-5xl font-black mb-3">
                <span className="text-primary">{t("schoolsPage.title")}</span>
              </h1>
              <p className="text-muted-foreground text-lg">{t("schoolsPage.description")}</p>
            </div>
            <Badge variant="outline" className="text-sm">
              {schoolCards.length} {t("schoolsPage.count")}
            </Badge>
          </motion.div>
          ...
          {(isLoadingOrgs || isLoadingGrades) ? (
            ...
            <p className="text-center text-muted-foreground py-12">{t("schoolsPage.empty")}</p>
          ) : (
                ...
                <Link key={org.id} to={`/org/${org.slug}`}>
                  <div className="p-2 text-center space-y-3 transition-transform hover:-translate-y-0.5">
                      ...
                      <div className="space-y-1">
                        ...
                        <Badge variant="secondary" className="text-[10px]">{t("common.school")}</Badge>
                      </div>
