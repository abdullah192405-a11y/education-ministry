...
const SingleChallenge = () => {
    ...
    const handleShare = () => {
        ...
        const scoreEmoji = results.percentage >= 90 ? "\u{1F3C6}" : results.percentage >= 75 ? "\u{2B50}" : "\u{1F4AA}";
        
        const shareText = [
            `\u{1F680} *تحدي تم إنجازه بنجاح!* ${scoreEmoji}`,
            `━━━━━━━━━━━━━━`,
            `\u{1F4DA} *الدرس:* ${topic?.title || 'تحدي عام'}`,
            `\u{1F3AF} *النتيجة:* ${results.percentage}%`,
            `\u{1F4B0} *النقاط:* ${results.score} نقطة`,
            `\u{2705} *الإجابات الصحيحة:* ${results.correctAnswers} من ${results.totalQuestions}`,
            `\u{23F1}\u{FE0F} *الوقت المستغرق:* ${Math.round(results.timeTaken)} ثانية`,
            results.longestStreak >= 2 ? `\u{1F525} *سلسلة نارية:* ${results.longestStreak} إجابات متتالية!` : ``,
            results.badges.length > 0 ? `\u{1F3C5} *الشارات المكتسبة:* ${results.badges.map(b => b.name).join(" | ")}` : ``,
            `━━━━━━━━━━━━━━`,
            `هل يمكنك تحطيم رقمي القياسي؟ \u{1F60F}\u{1F447}`
        ].filter(Boolean).join("\n");
